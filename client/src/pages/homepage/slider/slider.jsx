...
function Slider(reqs) {
  ...
  return (
    <AutoplaySlider
      ...
      className="Slider"
    >
      {content.slice(0, 4).map((x) => (
        <div
          className="contenaire"
          style={{
            backgroundImage: `url(${x.banner})`,
            backgroundSize: "cover",
          }}
        >
          <div>
            <h1 className="movie_title">{x.title}</h1>
            <div className="gap flex">
              <button className="quality">{x.quality}</button>
              <div className="rating flex">
                <button className="star_icon" />
                <button className="rating_num">{x.rating}</button>
              </div>
              <button className="length">{x.duration}</button>
              <div className="genres flex">
                {x.genre.map((i) => (
                  <button className="genre">{i}</button>
                ))}
              </div>
            </div>
